...
export const App = ({ position = [0, 0, 2.5], fov = 25 }) => (
  <Canvas camera={{ position, fov }} gl={{ preserveDrawingBuffer: true }} eventSource={document.getElementById('root')} eventPrefix="client">
  <hemisphereLight
  skyColor="#ffffff"
  groundColor="#dddddd"
  intensity={1}
...
function Shirt(props) {
  ...
  const { nodes } = useGLTF('/shirt_baked_collapsed.glb')

  return (
    <mesh geometry={nodes.T_Shirt_male.geometry} scale={1.6} position={[2.4, 0, 0]} castShadow receiveShadow={false}>
      <meshStandardMaterial
        color={snap.color}
        roughness={0.4}
        metalness={0}
      />
      <Decal position={[0, 0.04, 0.15]} scale={0.15} map={texture} />
    </mesh>
  )
}
